...
|                                                                         |
|   You should have received a copy of the GNU General Public License     | //ciao
...
namespace OpenSMOKE
{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::WriteASCIIFile(const std::string file_name) const
	{
		...
		unsigned int number_of_extendedplogopt_reactions = 0; // AB //
		...
		for (std::vector<ReactionPolicy_CHEMKIN>::const_iterator it = reactions_.begin(); it != reactions_.end(); ++it)
		{
			...
			else if ( (*it).Tag() == PhysicalConstants::REACTION_SIMPLE)
			{
				...
				else if ((*it).IsExtendedPressureLogOpt() == true)	number_of_extendedplogopt_reactions++; // AB // extended plog
		...
		std::vector<unsigned int> indices_of_extendedpressurelogopt_reactions(number_of_extendedplogopt_reactions);
		...
		// AB // extended-pressurelogopt-reactions //
		{ 
			unsigned int count = 0;
			for (std::vector<ReactionPolicy_CHEMKIN>::const_iterator it = reactions_.begin(); it != reactions_.end(); ++it)
				if ((*it).Tag() == PhysicalConstants::REACTION_SIMPLE && (*it).IsExtendedPressureLogOpt() == true) indices_of_extendedpressurelogopt_reactions[count++] = it - reactions_.begin() + 1;

			fOutput << "extended-pressurelogopt-reactions" << std::endl;
			fOutput << number_of_extendedplogopt_reactions << std::endl;
			for (unsigned int i = 0; i<number_of_extendedplogopt_reactions; i++)
				fOutput << indices_of_extendedpressurelogopt_reactions[i] << " ";
			fOutput << std::endl;
		}
		// AB // extended-pressurelogopt-reactions //
		{
		...
		{
			fOutput << "extended-pressurelogopt-parameters " << std::endl;
			for (unsigned int j = 0; j<number_of_extendedplogopt_reactions; j++)
				reactions_[indices_of_extendedpressurelogopt_reactions[j] - 1].WriteAdditionalDataOnASCIIFile(fOutput);
		}
		{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::WriteXMLFile(std::stringstream& fOutput) const
	{
		...
		unsigned int number_of_pressurelog_reactions = 0;
		unsigned int number_of_extendedpressurelog_reactions = 0;
		unsigned int number_of_extendedpressurelogopt_reactions = 0; // AB added the number of extended plog reactions for optimization
		unsigned int number_of_extendedfalloff_reactions = 0;
		unsigned int number_of_fit1_reactions = 0;
		unsigned int number_of_janevlanger_reactions = 0;
		unsigned int number_of_landauteller_reactions = 0;
		...
		for (std::vector<ReactionPolicy_CHEMKIN>::const_iterator it = reactions_.begin(); it != reactions_.end(); ++it)
		{
			...
			else if ( (*it).Tag() == PhysicalConstants::REACTION_SIMPLE)
			{
				...
				if ((*it).IsExtendedPressureLogOpt() == true)	number_of_extendedpressurelogopt_reactions++; // AB // added the check on the number of extended pressurelogopt //
				else if ( (*it).IsJanevLanger() == true )	number_of_janevlanger_reactions++;
				else if ( (*it).IsFit1() == true )			number_of_fit1_reactions++;
				else if ( (*it).IsLandauTeller() == true )	number_of_landauteller_reactions++;
			}
		...
		std::vector<unsigned int> indices_of_pressurelog_reactions(number_of_pressurelog_reactions);
		std::vector<unsigned int> indices_of_extendedpressurelog_reactions(number_of_extendedpressurelog_reactions);
		std::vector<unsigned int> indices_of_extendedpressurelogopt_reactions(number_of_extendedpressurelogopt_reactions); // AB // added the vector of indices on the number of extended pressurelogopt //
		std::vector<unsigned int> indices_of_extendedfalloff_reactions(number_of_extendedfalloff_reactions);
		std::vector<unsigned int> indices_of_janevlanger_reactions(number_of_janevlanger_reactions);
		std::vector<unsigned int> indices_of_fit1_reactions(number_of_fit1_reactions);
		std::vector<unsigned int> indices_of_landauteller_reactions(number_of_landauteller_reactions);
...

		{
			unsigned int count = 0;
			for (std::vector<ReactionPolicy_CHEMKIN>::const_iterator it = reactions_.begin(); it != reactions_.end(); ++it)
				if ((*it).Tag() == PhysicalConstants::REACTION_SIMPLE && (*it).IsExtendedPressureLogOpt() == true) indices_of_extendedpressurelogopt_reactions[count++] = boost::lexical_cast<int>(it - reactions_.begin() + 1); // AB //

			FormatXML(fOutput, "ExtendedPressureLogOpt", indices_of_extendedpressurelogopt_reactions); // AB //
		}
		...
		
		{
			...
			// actually print the coefficients to extendedpressureLogOpt
			if (number_of_extendedpressurelogopt_reactions != 0)
			{
				fOutput << "<ExtendedPressureLogOpt>" << std::endl;
				for (unsigned int j = 0; j<number_of_extendedpressurelogopt_reactions; j++)
					reactions_[indices_of_extendedpressurelogopt_reactions[j] - 1].WriteAdditionalDataOnASCIIFile(fOutput);
				fOutput << "</ExtendedPressureLogOpt>" << std::endl;
			}
			// actually print the coefficients to extendedpressureLogOpt
