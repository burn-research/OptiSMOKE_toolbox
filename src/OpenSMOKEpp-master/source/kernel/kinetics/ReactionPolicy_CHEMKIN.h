...
namespace OpenSMOKE
{
	...
	*/
	...
	public:
		...
		*@brief Is the reaction an extended pressure logarithmic interpolated?
		*/
		bool IsExtendedPressureLogOpt() const { return iExtPlogOpt_; }
		...
		*/
		...
		/**
 		*@brief Returns the plog coefficients
 		*/
		const std::vector<double>& plog_coefficients() const {return plog_coefficients_;}

		/**
 		*@brief Returns the janev langer coefficients
 		*/
		const std::vector<double>& janev_langer_coefficients() const {return janev_langer_coefficients_;}

		/**
 		*@brief Returns the chebyshev coefficients
 		*/
		const std::vector<double>& chebyshev_coefficients() const {return chebyshev_coefficients_;}

		/**
 		*@brief Returns the chebyshev pressure limits
 		*/
		const std::vector<double>& chebyshev_pressure_limits() const {return chebyshev_pressure_limits_;}

		/**
 		*@brief Returns the chebyshev temperature limits
 		*/
		const std::vector<double>& chebyshev_temperature_limits() const {return chebyshev_temperature_limits_;}

		/**
 		*@brief Returns the extended fall off coefficients
 		*/
		const std::vector< std::vector<std::string>>& extendedfalloff_coefficients() const {return extendedfalloff_coefficients_;}

		/**
 		*@brief Returns the extended plog coefficients
 		*/
		const std::vector<std::string>& extendedplog_coefficients() const {return extendedplog_coefficients_;}

		/**
 		*@brief Returns the extended plog coefficients
 		*/
		const std::vector<std::string>& extendedplogopt_coefficients() const {return extendedplogopt_coefficients_;}
		

		/**
                *@brief Returns the landau teller coefficients
                */
                const std::vector<double>& landau_teller_coefficients() const {return landau_teller_coefficients_;}

		/**
                *@brief Returns the fit1 coefficients
                */
                const std::vector<double>& fit1_coefficients() const {return fit1_coefficients_;}

		/**
                *@brief Returns the TROE coefficients
                */
                const std::vector<double>& troe() const {return troe_;}

		/**
                *@brief Returns the SRI coefficients
                */
                const std::vector<double>& sri() const {return sri_;}
	...
	protected:
		...
		std::vector<std::string> extendedplog_coefficients_;	//!< coefficients
				...
				// Extended Pressure Dependence Through Logarithmic Interp.
		bool iExtPlogOpt_;											//!< is a Pressure Dependence Through Logarithmic Interp. reaction (PLOG)?
		std::vector<std::string> extendedplogopt_coefficients_;	//!< coefficients
