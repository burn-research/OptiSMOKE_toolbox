...
#ifndef OpenSMOKE_Solve_Dense_KinSol_H
#define OpenSMOKE_Solve_Dense_KinSol_H
...
namespace NlsSMOKE
{
	...
	int Solve_Dense_KinSol(	Object* object, const NlsSMOKE::NonLinearSolver_Parameters& parameters,
							Eigen::VectorXd& yeig, Eigen::VectorXd& feig)
	{
		std::cout << "Dense NLS solution (KinSol)..." << std::endl;
		...
		if (flag_solver >= 0)
		{
			...
			// Move from Sundials to Eigen
			yeig.resize(neq);
			feig.resize(neq);
			for (int i = 0; i < neq; i++)
			{
				yeig(i) = NV_Ith_S(y, i);
				feig(i) = -1.;// NV_Ith_S(f, i);
			}
		}
		else
		{
	...
	static void KinSolStatistics(void *kmem)
	{
		...
		// Dense solver statistics
		{
			...
			std::cout << "Dense linear solver statistics.. " << std::endl;
		...
		// Workspace sizes
		{
			...
			// Dense linear solver workspace size
...
#endif // OpenSMOKE_Solve_Dense_KinSol_H
