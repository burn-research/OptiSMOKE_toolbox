...
namespace OpenSMOKE
{ 
	KineticsMap_Liquid_CHEMKIN::KineticsMap_Liquid_CHEMKIN(ThermodynamicsMap_Liquid_CHEMKIN& thermo, rapidxml::xml_document<>& doc, const unsigned int target) :
	...
	KineticsMap_Liquid_CHEMKIN::KineticsMap_Liquid_CHEMKIN(ThermodynamicsMap_Liquid_CHEMKIN& thermo, rapidxml::xml_document<>& doc, const std::string target) :
	thermodynamics_(thermo)
	{
		ImportSpeciesFromXMLFile(doc);
		ImportCoefficientsFromXMLFile(doc, target);
	...
	void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)
	{
		ErrorMessage("void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)", "The liquid kinetic map require the user specifies tha material name.");
	}

	void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc, const std::string target_material_name)
	{
		...
		if (kinetics_type != "OpenSMOKE" || kinetics_version != "01-02-2014")
			ErrorMessage("void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)", "The current kinetic scheme is not supported.");
		...
		if (target_material_index == 0)
			ErrorMessage("void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)", "The requested liquid material is not available. Please check the name.");
		else
			ImportCoefficientsFromXMLFile(doc, target_material_index);
	}
 
	void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc, const unsigned int target_material_index)
	{
		...
		for (rapidxml::xml_node<> *kinetics_node = parent_kinetics_node->first_node("MaterialKinetics"); kinetics_node; kinetics_node = kinetics_node->next_sibling("MaterialKinetics"))
		{
			...
			{
				if (kinetics_node == 0)
					ErrorMessage("void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)", "Kinetics tag was not found!");
				...
				std::cout << " * Reading kinetic parameters of reactions..." << std::endl;	
				{
					...
					// Direct side
					{
						...
						// Global kinetic order of forward reaction
						{
							...
							std::stringstream fInput;
							fInput << current_node->value();
							Load(forward_kinetic_order__, fInput, OPENSMOKE_FORMATTED_FILE);
						}
					}
				...
				// Stoichiometry
				{
					...
					if (stoichiometry_type != "OpenSMOKE" || stoichiometry_version != "01-02-2014")
						ErrorMessage("void KineticsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)", "The current stoichiometric data are not supported.");
	...
	void KineticsMap_Liquid_CHEMKIN::ImportSpeciesFromXMLFile(rapidxml::xml_document<>& doc)
	{
		rapidxml::xml_node<>* opensmoke_node = doc.first_node("opensmoke");
		rapidxml::xml_node<>* number_of_species_node = opensmoke_node->first_node("NumberOfSpecies");
		try
		...
		{
			ErrorMessage("KineticsMap_Liquid_CHEMKIN::ImportSpeciesFromXMLFile", "Error in reading the number of species.");
		}
	}
