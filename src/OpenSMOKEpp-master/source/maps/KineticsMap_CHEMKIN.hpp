...
namespace OpenSMOKE
{
	...
	/* HERE HE MANAGES the KINETIC MAP, in particular he is copying from rhs into the variables that he created 
	...
	/* In particular he references to the pointer of KineticsMap_CHEMKIN and assign it to rhs, secondly from rhs
	 he copies values into different variables */
	void KineticsMap_CHEMKIN::CopyFromMap( const KineticsMap_CHEMKIN& rhs)
    {
		...
		// AB // 
		this->indices_of_extendedpressurelogopt_reactions__ = rhs.indices_of_extendedpressurelogopt_reactions__;
		// AB //
		...
		// AB // 
		this->number_of_extendedplogopt_reactions_ = rhs.number_of_extendedplogopt_reactions_;
		// AB //
		...
		// AB // extendedpressure dependent reaction for optimization
		this->extendedplogopt_reactions_ = new Optimization_EPLE[this->number_of_extendedplogopt_reactions_];
		for (unsigned int j = 0; j<this->number_of_extendedplogopt_reactions_; j++)
			this->extendedplogopt_reactions_[j] = rhs.extendedplogopt_reactions_[j];
		// AB //
	...
	KineticsMap_CHEMKIN::~KineticsMap_CHEMKIN()
	{
		...
		if (number_of_extendedplogopt_reactions_ > 0)
			delete[] extendedplogopt_reactions_;
	...
	void KineticsMap_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)
	{
		...
		// ExtendedPressureLog reactions for optimization
		{
			rapidxml::xml_node<>* current_node = kinetics_node->first_node("ExtendedPressureLogOpt");
			number_of_extendedplogopt_reactions_ = 0;
			
			if (current_node != 0)
			{
				std::stringstream fInput;
				fInput << current_node->value();
				Load(indices_of_extendedpressurelogopt_reactions__, fInput, OPENSMOKE_FORMATTED_FILE);
				number_of_extendedplogopt_reactions_ = indices_of_extendedpressurelogopt_reactions__.size();
			}
		}
		    ...
		    std::cout << " * Reading kinetic parameters of reactions..." << std::endl;	
		{
			    ...
			    std::cout << " * Reading kinetic parameters of additional reactions..." << std::endl;
			{
				...
				if (number_of_extendedpressurelog_reactions_ != 0)
				{
					//here it is actually assigning the node "ExtendedFallOff" to the pointer current node
					rapidxml::xml_node<>* current_node = kinetic_parameters_node->first_node("ExtendedPressureLog");
					// defines a buffer fInput
					std::stringstream fInput;
					// assigns the value of the current node to the buffer
					fInput << current_node->value();
					// allocates dinamically new memory for extendedpressurelog from the heap
				...
				// Extended PressureLog reaction parameters for Optimization // AB //
				if (number_of_extendedplogopt_reactions_ != 0)
				{
					//here it is actually assigning the node "ExtendedFallOff" to the pointer current node
					rapidxml::xml_node<>* current_node = kinetic_parameters_node->first_node("ExtendedPressureLogOpt");
					// defines a buffer fInput
					std::stringstream fInput;
					// assigns the value of the current node to the buffer
					fInput << current_node->value();
					// allocates dinamically new memory for extendedpressurelog from the heap
					extendedplogopt_reactions_ = new Optimization_EPLE[number_of_extendedplogopt_reactions_];
					
					for (unsigned int j = 0; j < number_of_extendedplogopt_reactions_; j++)
						extendedplogopt_reactions_[j].ReadFromASCIIFile(fInput);
				}
		...
		// Stoichiometry
		{
			...
			if (verbose_output_ == true)
			{
				...
				std::cout << " Ext-Pressure-Log reactions OPT: " << number_of_extendedplogopt_reactions_ << " (" << number_of_extendedplogopt_reactions_ / std::max(1., double(this->number_of_reactions_))*100. << "%)" << std::endl;
	...
	// Extended Pressure logarithmic interpolated reactions for Optimization // AB //
	void KineticsMap_CHEMKIN::ExtendedPressureLogReactionsForOptimization(const double cTot, const double* c)
	{
		for (unsigned int k = 0; k < number_of_extendedplogopt_reactions_; k++)
		{
			const unsigned int j = indices_of_extendedpressurelogopt_reactions__[k]-1;
			kArrhenius__[j] = extendedplogopt_reactions_[k].KineticConstant(this->T_, this->P_, cTot, c);
			kArrheniusModified__[j] = kArrhenius__[j];
		}
	}
	...
	void KineticsMap_CHEMKIN::ReactionRates(const double* c, const double cTot)
	{
		...
		// 1.bisbis Extended pressure log reactions for optimization
		ExtendedPressureLogReactionsForOptimization(cTot, c);
