...
{ 
	...
	void KineticsMap_Solid_CHEMKIN::ReactionRates(const double* cGas, const double* cSolid)
	{
		double cTot = 0.;
...
//			reverseReactionRates_[j] *= kArrhenius_reversible_[k-1]/kArrheniusModified__[j-1];
	...
	void KineticsMap_Solid_CHEMKIN::FormationRates(double* Rgas, double* Rsolid)
	{
	...
	double KineticsMap_Solid_CHEMKIN::HeatRelease(const double* Rgas, const double* RSolid)
	{
		unsigned int k = 0;
		for (unsigned int j = 0; j<thermodynamics_.number_of_gas_species(); j++)
			aux_vector__[k++] = Rgas[j];
		for (unsigned int j = 0; j < thermodynamics_.number_of_solid_species(); j++)
			aux_vector__[k++] = RSolid[j];

		return -Dot(aux_vector__.size(), aux_vector__.data(), thermodynamics_.Species_H_over_RT().data()) * PhysicalConstants::R_J_kmol * this->T_;
	}

	void KineticsMap_Solid_CHEMKIN::ProductionAndDestructionRates(double* P, double* D)
	{
		stoichiometry_->ProductionAndDestructionRatesFromReactionRates(P, D, netReactionRates__.data());
	}
	
	const std::vector<double>& KineticsMap_Solid_CHEMKIN::GiveMeReactionRates()
	{
		return netReactionRates__;
	}

	void KineticsMap_Solid_CHEMKIN::GiveMeReactionRates(double* r)
	{
		r = netReactionRates__.data();
	}

	void KineticsMap_Solid_CHEMKIN::GetForwardReactionRates(double* r)
	{
		ElementByElementProduct(forwardReactionRates__.size(), forwardReactionRates__.data(), kArrheniusModified__.data(), r);
	}
 
	void KineticsMap_Solid_CHEMKIN::GetBackwardReactionRates(double* r)
	{
		for (unsigned int j = 0; j < this->number_of_reactions_; j++)
			r[j] = 0.;
	...
	void KineticsMap_Solid_CHEMKIN::WriteKineticData(std::ostream& fOut, const unsigned int k, const double* c_bath, const double conversion_forward, const double conversion_backward)
	{			
		// TODO
	}

	void KineticsMap_Solid_CHEMKIN::FittedReverseKineticConstants(const double* x_bath, const unsigned int nparameters, Eigen::MatrixXd& fittedKineticParameters)
	{			
		// TODO
	}
