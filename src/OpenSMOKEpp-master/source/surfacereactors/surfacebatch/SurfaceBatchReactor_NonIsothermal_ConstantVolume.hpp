...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_SurfaceBatchReactor* ptOde_SurfaceBatchReactor_NonIsothermal_ConstantVolume;
	void ODE_Print_SurfaceBatchReactor_NonIsothermal_ConstantVolume(BzzVector &Y, double t)
	{
		ptOde_SurfaceBatchReactor_NonIsothermal_ConstantVolume->MyPrint(Y,t);
	}
	#endif

	SurfaceBatchReactor_NonIsothermal_ConstantVolume::SurfaceBatchReactor_NonIsothermal_ConstantVolume(	
	...
	{
		type_ = SURFACEBATCH_REACTOR_NONISOTHERMAL_CONSTANTV;
		...
		NE_ = NC_ + SURF_NC_ + SURF_NP_ + 1 + 1 + BULK_NC_;
	...
	int SurfaceBatchReactor_NonIsothermal_ConstantVolume::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		// Recover unknowns
		{
			unsigned int k=1;
		...
		const double dm_over_dt = A_*Dot(RfromSurface_.Size(), RfromSurface_.GetHandle(), thermodynamicsMap_.MWs().data());

		// Number of moles
		const double moles = cTot_*V_;
		const double sumMoleFormationRates = V_*RfromGas_.SumElements() + A_*RfromSurface_.SumElements();
		...
		// Recovering residuals
		{
			...
			// Energy equation
			dy[k++] = (V_*QRGas_+ A_*QRSurface_+PhysicalConstants::R_J_kmol*T_*sumMoleFormationRates) / 
	...
	int SurfaceBatchReactor_NonIsothermal_ConstantVolume::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
	...
	void SurfaceBatchReactor_NonIsothermal_ConstantVolume::Solve(const double tf)
	{
		if (batch_options_.verbose_output() == true)
		{
			std::cout << std::endl;
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			ptOde_SurfaceBatchReactor_NonIsothermal_ConstantVolume = &odebatch;
			o.StepPrint(ODE_Print_SurfaceBatchReactor_NonIsothermal_ConstantVolume);
	...
	void SurfaceBatchReactor_NonIsothermal_ConstantVolume::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		FatalErrorMessage("SurfaceBatchReactor_NonIsothermal_ConstantVolume::SensitivityAnalysis not yet implemented");
		...
		else
		{
			...
			scaling_Jp[NC_+1] = 1./(rho_*CvMixMass_);
