...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_SurfacePerfectlyStirredReactor* ptOde_SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure;
	void ODE_Print_SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure(BzzVector &Y, double t)
	{
		ptOde_SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure->MyPrint(Y,t);
	}
	#endif

	SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure(	
	...
	{
		OpenSMOKE::FatalErrorMessage("Non isothermal surface CSTR not yet implemented!");

		type_ = SURFACEPERFECTLYSTIRRED_REACTOR_NONISOTHERMAL_CONSTANTP;
		...
		NE_ = NC_ + 1 + 1 +SURF_NP_ + SURF_NC_ + BULK_NC_;
	...
	int SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		...
		for (unsigned int i=1;i<=NC_;++i)	
			dy[i] = (omegaInlet_[i]-omega_[i])/tau_ + thermodynamicsMap_.MW(i-1)*RfromGas_[i]/rho_;
		...
		dy[NC_+1]  = ( (HInlet_-HStarI)/MWInlet_/tau_ + QRGas_/rho_ + Qexchange/(rho_*V_)) / CpMixMass_;

		return 0;
	}

	int SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
		...
		if (psr_options_.verbose_video() == true)
		{
			...
			if (iteration_%psr_options_.n_step_video() == 1 || psr_options_.n_step_video() == 1)
			{
				...
				if (counter_file_video_%100 == 1)
				{
					...
					std::cout << std::setw(14) << std::left << "m(in)[g/s]";
					std::cout << std::setw(14) << std::left << "m(out)[g/s]";
					std::cout << std::setw(14) << std::left << "m(loss)[g/s]";
					std::cout << std::endl;
				}
	...
	void SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::PrintFinalStatus(std::ostream& fOutput)
	{
	...
	void SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::PrintParametricFinalStatus(std::ostream& fOutput)
	{
		fOutput.setf(std::ios::scientific);
		fOutput << std::setw(20) << std::left << tau_;
		fOutput << std::setw(20) << std::left << T0_;
	...
	void SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::PrintSolution()
	{
		if (psr_options_.verbose_output() == true)
		{
			// ASCII file output
	...
	void SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::Solve(const double tf)
	{
		if (psr_options_.verbose_video() == true)
		{
			std::cout << std::endl;
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			ptOde_SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure = &odepsr;
			o.StepPrint(ODE_Print_SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure);
	...
	void SurfacePerfectlyStirredReactor_NonIsothermal_ConstantPressure::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		// Scaling factors
