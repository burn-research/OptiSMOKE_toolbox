...
void SimulationsInterface::Setup()
{
    ...
    if (n_psr != 0)
    {
        ...
        for (unsigned int i = 0; i < n_psr; i++)
            perfectlystirred_reactors[i] =
                new OptiSMOKE::PerfectlyStirredReactor[data_.input_paths()[i + offset].size()];
    ...
    for (int j = 0; j < rows; j++)
    {
        ...
        for (int i = 0; i < columns; i++)
        {
            ...
            k_upper[j][i] = k_0[j][i] * std::pow(10, f_factors[j]) *
                            ((double)data_.optimization_setup().sigma_k_distribution() / 2);
            k_lower[j][i] = k_0[j][i] * std::pow(10, -1 * f_factors[j]) /
                            ((double)data_.optimization_setup().sigma_k_distribution() / 2);
        }
    ...
    for (unsigned int j = 0; j < rows_cp; j++)
    {
        ...
        int pos_classic_plog_reaction =
            std::find(indices_of_classic_plog.begin(), indices_of_classic_plog.end(),
                      data_.optimization_target().list_of_target_classic_plog_reactions()[j]) -
            indices_of_classic_plog.begin();

        for (int k = 0;
             k < data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size(); k++)
        {
            if (data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA()[k].size() != 1)
            {
                std::cout << " * WARNING the plog reactions requested for the optimization is a duplicate!"
                          << std::endl;
            ...
            for (unsigned int j = 0; j < data_.input_paths()[i].size(); j++)
            {
                ...
                if (qoi == "IDT")
                {
                    if (reactor_mode == "shock tube")
                        simulations_results_[i][0][j] =
                            batch_reactors[i][j].GetIgnitionDelayTime(qoi_target) * std::pow(10, 6);
                ...
                else if (qoi == "Composition")
                {
                    OptiSMOKE::FatalErrorMessage(
                        "Compositions profile measurements in batch reactors not yet implemented!");
                }
                else
                {
                ...
                if (qoi == "Composition")
                {
                    if (qoi_target == "mole-fraction-out")
                    {
                        std::vector<double> tmp =
                            plugflow_reactors[i - n_batch][j].GetMolefractionsOut(data_.ordinates_label()[i]);
        ...
        if (solver == "PerfectlyStirredReactor")
        {
            for (unsigned int j = 0; j < data_.input_paths()[i].size(); j++)
            {
                ...
                if (qoi == "Composition")
                {
                    if (qoi_target == "mole-fraction-out")
                    {
                        std::vector<double> tmp = perfectlystirred_reactors[i - n_batch - n_pfr][j].GetMolefractionsOut(
                            data_.ordinates_label()[i]);
                        for (unsigned int k = 0; k < data_.ordinates_label()[i].size(); k++)
                        {
                            if (data_.ordinates_label()[i][k] == "DeltaTemp")
                                simulations_results_[i][k][j] =
                                    tmp[k] - data_.expdata_x()[i][k][j]; // Check if it is negative?
                            else
                                simulations_results_[i][k][j] = tmp[k];
                        }
                    }
...
double SimulationsInterface::ComputeObjectiveFunction()
{
        ...
        for (unsigned int i = 0; i < simulations_results_.size(); i++)
        {
            ...
            for (unsigned int j = 0; j < simulations_results_[i].size(); j++)
            {
                ...
                if (data_.QoI()[i] == "IDT")
                {
                    ...
                    CM_indexes =
                        curveMatching(data_.curvematching_options().number_of_bootstrap(), data_.expdata_x()[i][j], tmp,
                                      data_.expdata_x()[i][j], tmp2, data_.uncertainty()[i][j]);
                }
                else
                {
                    CM_indexes =
                        curveMatching(data_.curvematching_options().number_of_bootstrap(), data_.expdata_x()[i][j],
    ...
    {
        OptiSMOKE::FatalErrorMessage(
            "Unknown type of objective function. Available are: L1-norm | L2-norm | CurveMatching");
    }
...
bool SimulationsInterface::CheckKineticConstasts()
{
    ...
    for (int j = 0; j < data_.optimization_target().list_of_uncertainty_factors_classic_plog().size(); j++)
    {
        ...
        for (int k = 0; k < data_.kineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size(); k++)
        {
            ...
            {
                k_check_CP[i] =
                    A_CP_trial * std::pow(T_span[i], n_CP_trial) * std::exp((-1 * E_over_R_CP_trial) / T_span[i]);

                if ((k_check_CP[i] <= k_lower_classic_plog[j][k][i]) ||
                    (k_check_CP[i] >= k_upper_classic_plog[j][k][i]))
                {
...
void SimulationsInterface::SubstituteKineticParameters(const std::vector<double> &c_vars)
{
    ...
    if (data_.optimization_target().list_of_target_E_over_R_inf().size() != 0)
    {
        ...
        {
            ChangeFallOffParamaters("E_over_R", data_.optimization_target().list_of_target_E_over_R_inf()[i],
                                    c_vars[count]);
            count += 1;
        }
    }
    ...
    if (data_.optimization_target().list_of_target_classic_plog_reactions().size() != 0)
    {
        ...
        {
            ChangePLOGReactions("lnA", data_.optimization_target().list_of_target_classic_plog_reactions()[i],
                                c_vars[count]);
            count += 1;
        }
        // E_over_R
...
void SimulationsInterface::ChangePLOGReactions(std::string type, unsigned int index, double parameter)
{
    ...
    if (type == "lnA")
    {
        ...
        {
            double lnA_nominal =
                data_.nominalkineticsMapXML_->pressurelog_reactions(pos_classic_plog_reaction).lnA()[k][0];
    ...
    if (type == "Beta")
    {
        ...
        {
            double beta_nominal =
                data_.nominalkineticsMapXML_->pressurelog_reactions(pos_classic_plog_reaction).Beta()[k][0];
