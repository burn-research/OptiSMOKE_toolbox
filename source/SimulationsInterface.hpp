...
void SimulationsInterface::Setup() {
  ...
  if (n_premixed != 0) {
    premixed1D.resize(n_premixed);
    for (unsigned int i = 0; i < n_premixed; i++) {
      premixed1D[i] = new OptiSMOKE::PremixedLaminarFlame1D[data_.input_paths()[i + offset].size()];
    }
  ...
  for (int j = 0; j < rows; j++) {
    ...
    for (int i = 0; i < columns; i++) {
      ...
      k_upper[j][i] =
          k_0[j][i] * std::pow(10, f_factors[j]) * ((double)data_.optimization_setup().sigma_k_distribution() / 2);
      k_lower[j][i] =
          k_0[j][i] * std::pow(10, -1 * f_factors[j]) / ((double)data_.optimization_setup().sigma_k_distribution() / 2);
    }

    // std::cout << "Reaction " << reaction_index << ": " << std::endl;
    // for (int i=0; i < T_span.size(); i++)
    // {
    //     std::cout<< "The value of f for the lower constraint is: " << k_lower[j][i]
    //     <<std::endl;
    // }

    // for (int i=0; i < T_span.size(); i++)
    // {
    //     std::cout<< "The value of f for the upper constraint is: " << k_upper[j][i]
    //     <<std::endl;
    // }
  }
  // }
  ...
  for (int j = 0; j < rows_inf; j++) {
    ...
    int pos_FallOff_Reaction =
        std::find(indices_of_falloff_reactions.begin(), indices_of_falloff_reactions.end(), falloff_reaction_index) -
    ...
    for (int i = 0; i < columns_inf; i++) {
      ...
      k_upper_inf[j][i] = k_0_inf[j][i] * std::pow(10, f_factors_inf[j]) *
                          (double)data_.optimization_setup().sigma_k_distribution() / 2;
      k_lower_inf[j][i] = k_0_inf[j][i] * std::pow(10, -1 * f_factors_inf[j]) /
                          ((double)data_.optimization_setup().sigma_k_distribution() / 2);
    }
  }
  ...
  for (unsigned int j = 0; j < rows_cp; j++) {
    ...
    int pos_classic_plog_reaction = std::find(indices_of_classic_plog.begin(), indices_of_classic_plog.end(),
                                              data_.optimization_target().list_of_target_classic_plog_reactions()[j]) -
                                    indices_of_classic_plog.begin();

    for (int k = 0; k < data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size();
         k++) {
      if (data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA()[k].size() != 1) {
        std::cout << " * WARNING the plog reactions requested for the optimization is a "
                     "duplicate!"
                  << std::endl;
        std::cout << "   * Reaction: " << data_.optimization_target().list_of_target_classic_plog_reactions()[j]
                  << std::endl;
      }
    }

    k_classic_plog[j].resize(
        data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size());
    k_upper_classic_plog[j].resize(
        data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size());
    k_lower_classic_plog[j].resize(
        data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size());

    for (int k = 0; k < data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size();
         k++) {
      double A_CP =
          std::exp(data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA()[k][0]);
      double n_CP = data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).Beta()[k][0];
      double E_over_R_CP =
          data_.nominalkineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).E_over_R()[k][0];
      ...
      for (int i = 0; i < T_span.size(); i++) {
        ...
        k_upper_classic_plog[j][k][i] =
            k_classic_plog[j][k][i] *
            std::pow(10, data_.optimization_target().list_of_uncertainty_factors_classic_plog()[j]) *
            ((double)data_.optimization_setup().sigma_k_distribution() / 2);
        k_lower_classic_plog[j][k][i] =
            k_classic_plog[j][k][i] *
            std::pow(10, -1 * data_.optimization_target().list_of_uncertainty_factors_classic_plog()[j]) /
            ((double)data_.optimization_setup().sigma_k_distribution() / 2);
      }
    }
  }
  ...
  // Compute max and min reaction rates according to temperature dependent
  // or constant uncertainty factors for RPBRM reactions
  std::vector<std::vector<std::vector<double>>> k_rpbrm;
  int rows_rpbrm = data_.optimization_target().list_of_uncertainty_factors_rpbmr().size();

  // resize first dimensions according to the number of classic plog we are interested in
  // for optimisation
  k_rpbrm.resize(rows_rpbrm);
  k_upper_rpbrm.resize(rows_rpbrm);
  k_lower_rpbrm.resize(rows_rpbrm);

  // ==============
  // TITO TMP STOP
  // ==============
  // for (unsigned int j = 0; j < rows_rpbrm; j++) {
  //   // finding the position of the target reaction in correspondent OS++ object
  //   int pos_rpbrm_reaction =
  //       std::find(data_.nominalkineticsMapXML()->IndicesOfRPBMR().begin(),
  //                 data_.nominalkineticsMapXML()->IndicesOfRPBMR().end(),
  //                 data_.optimization_target().list_of_target_rpbmr_reactions()[j]) -
  ...
  //   int iSpecies = data_.thermodynamicsMapXML_->IndexOfSpecies(
  //                      data_.optimization_target().list_of_target_rpbmr_bathgases()[j]) -
  //                  1;
  //
  //   int pos_rpbrm_coll =
  //       std::find(coll_idx.begin(), coll_idx.end(), iSpecies) - coll_idx.begin();
  //
  //   // std::cout <<
  //   // std::exp(data_.nominalkineticsMapXML()->rpbrm_reactions(pos_rpbrm_reaction).kappaMu_PLOG(0).lnA()[0][0])
  //   // << "\n"; std::cout <<
  //   // std::exp(data_.nominalkineticsMapXML()->rpbrm_reactions(pos_rpbrm_reaction).kappaMu_PLOG(0).lnA()[1][0])
  //   // << "\n"; std::cout <<
  //   // std::exp(data_.nominalkineticsMapXML()->rpbrm_reactions(pos_rpbrm_reaction).kappaMu_PLOG(0).lnA()[2][0])
  //   // << "\n"; std::cout <<
  //   // std::exp(data_.nominalkineticsMapXML()->rpbrm_reactions(pos_rpbrm_reaction).kappaMu_PLOG(0).lnA()[3][0])
  //   // << "\n";
  //
  //   k_rpbrm[j].resize(data_.nominalkineticsMapXML()
  //                         ->rpbrm_reactions(pos_rpbrm_reaction)
  //                         .kappaMu_PLOG(pos_rpbrm_coll)
  //                         .lnA()
  //                         .size());
  //   k_upper_rpbrm[j].resize(data_.nominalkineticsMapXML()
  //                               ->rpbrm_reactions(pos_rpbrm_reaction)
  //                               .kappaMu_PLOG(pos_rpbrm_coll)
  //                               .lnA()
  //                               .size());
  //   k_lower_rpbrm[j].resize(data_.nominalkineticsMapXML()
  //                               ->rpbrm_reactions(pos_rpbrm_reaction)
  //                               .kappaMu_PLOG(pos_rpbrm_coll)
  //                               .lnA()
  //                               .size());
  //
  //   for (int k = 0; k < data_.nominalkineticsMapXML()
  //                           ->rpbrm_reactions(pos_rpbrm_reaction)
  //                           .kappaMu_PLOG(pos_rpbrm_coll)
  //                           .lnA()
  //                           .size();
  //        k++) {
  //     double A    = std::exp(data_.nominalkineticsMapXML()
  //                                ->rpbrm_reactions(pos_rpbrm_reaction)
  //                                .kappaMu_PLOG(pos_rpbrm_coll)
  //                                .lnA()[k][0]);
  //     double Beta = data_.nominalkineticsMapXML()
  //                       ->rpbrm_reactions(pos_rpbrm_reaction)
  //                       .kappaMu_PLOG(pos_rpbrm_coll)
  //                       .Beta()[k][0];
  //     double E_over_R = data_.nominalkineticsMapXML()
  //                           ->rpbrm_reactions(pos_rpbrm_reaction)
  //                           .kappaMu_PLOG(pos_rpbrm_coll)
  //                           .E_over_R()[k][0];
  //
  ...
  //     for (int i = 0; i < T_span.size(); i++) {
  //       k_rpbrm[j][k][i] =
  //           A * std::pow(T_span[i], Beta) * std::exp((-1 * E_over_R) / T_span[i]);
  //       k_upper_rpbrm[j][k][i] =
  //           k_rpbrm[j][k][i] *
  //           std::pow(10, data_.optimization_target()
  //                            .list_of_uncertainty_factors_classic_plog()[j]) *
  //           ((double)data_.optimization_setup().sigma_k_distribution() / 2);
  //       k_lower_rpbrm[j][k][i] =
  //           k_rpbrm[j][k][i] *
  //           std::pow(10, -1 * data_.optimization_target()
  //                                 .list_of_uncertainty_factors_classic_plog()[j]) /
  //           ((double)data_.optimization_setup().sigma_k_distribution() / 2);
  //     }
  //   }
...
void SimulationsInterface::run() {
  ...
  for (unsigned int i = 0; i < data_.path_experimental_data_files().size(); i++) {
    ...
    if (solver == "BatchReactor") {
      for (unsigned int j = 0; j < data_.input_paths()[i].size(); j++) {
        ...
        if (qoi == "IDT") {
          ...
          else if (reactor_mode == "rapid compression machine") OptiSMOKE::FatalErrorMessage("RCM not yet implemented");
          else OptiSMOKE::FatalErrorMessage("Reactor mode is required for IDT experiments!");
        } else if (qoi == "Composition") {
          OptiSMOKE::FatalErrorMessage(
              "Compositions profile measurements in batch reactors not yet "
              "implemented!");
        } else {
        ...
        if (qoi == "Composition") {
          if (qoi_target == "mole-fraction-out") {
            std::vector<double> tmp =
                perfectlystirred_reactors[i - n_batch - n_pfr][j].GetMolefractionsOut(data_.ordinates_label()[i]);
            for (unsigned int k = 0; k < data_.ordinates_label()[i].size(); k++) {
              if (data_.ordinates_label()[i][k] == "DeltaTemp")
                simulations_results_[i][k][j] = tmp[k] - data_.expdata_x()[i][k][j];  // Check if it is negative?
              else simulations_results_[i][k][j] = tmp[k];
            }
          } else {
            OptiSMOKE::FatalErrorMessage("Unknown QoI target: " + qoi_target);
          }
    ...
    if (solver == "CounterFlowFlame1D") { OptiSMOKE::FatalErrorMessage(solver + " not supported yet!"); }
  }
}
...
double SimulationsInterface::ComputeObjectiveFunction() {
  ...
  if (data_.optimization_setup().objective_function_type() == "CurveMatching") {
    ...
    for (unsigned int i = 0; i < simulations_results_.size(); i++) {
      ...
      for (unsigned int j = 0; j < simulations_results_[i].size(); j++) {
        ...
        if (data_.QoI()[i] == "IDT") {
          ...
          CM_indexes = curveMatching(data_.curvematching_options().number_of_bootstrap(), data_.expdata_x()[i][j], tmp,
                                     data_.expdata_x()[i][j], tmp2, data_.uncertainty()[i][j]);
        } else {
          CM_indexes = curveMatching(data_.curvematching_options().number_of_bootstrap(), data_.expdata_x()[i][j],
                                     data_.expdata_y()[i][j], data_.expdata_x()[i][j], simulations_results_[i][j],
    ...
    for (int i = 0; i < CM_score.size(); i++) {
      ...
      if (std::isnan(i_th_index)) i_th_index = -1.;
    ...
    OptiSMOKE::FatalErrorMessage(
        "Unknown type of objective function. Available are: L1-norm | L2-norm | "
        "CurveMatching");
  }
...
bool SimulationsInterface::CheckKineticConstasts() {
  ...
  for (int j = 0; j < data_.optimization_target().list_of_target_uncertainty_factors_inf().size(); j++) {
    ...
    int pos_FallOff_Reaction = std::find(indices_of_falloff_reactions.begin(), indices_of_falloff_reactions.end(),
  ...
  for (int j = 0; j < data_.optimization_target().list_of_uncertainty_factors_classic_plog().size(); j++) {
    ...
    for (int k = 0; k < data_.kineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA().size(); k++) {
      double A_CP_trial =
          std::exp(data_.kineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).lnA()[k][0]);
      double n_CP_trial = data_.kineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).Beta()[k][0];
      double E_over_R_CP_trial =
          data_.kineticsMapXML()->pressurelog_reactions(pos_classic_plog_reaction).E_over_R()[k][0];
...
void SimulationsInterface::SubstituteKineticParameters(const std::vector<double> &c_vars) {
  ...
  if (data_.optimization_target().list_of_target_thirdbody_reactions().size() != 0) {
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_thirdbody_reactions().size(); i++) {
      ChangeThirdBodyEfficiencies(data_.optimization_target().list_of_target_thirdbody_reactions()[i],
                                  data_.optimization_target().list_of_target_thirdbody_species()[i], c_vars[count]);
      count += 1;
    }
  }
  ...
  if (data_.optimization_target().list_of_target_classic_plog_reactions().size() != 0) {
    ...
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_classic_plog_reactions().size(); i++) {
      ChangePLOGReactions("lnA", data_.optimization_target().list_of_target_classic_plog_reactions()[i], c_vars[count]);
      count += 1;
    }
    // E_over_R
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_classic_plog_reactions().size(); i++) {
      ChangePLOGReactions("E_over_R", data_.optimization_target().list_of_target_classic_plog_reactions()[i],
                          c_vars[count]);
      count += 1;
    }
    // Beta
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_classic_plog_reactions().size(); i++) {
      ChangePLOGReactions("Beta", data_.optimization_target().list_of_target_classic_plog_reactions()[i],
                          c_vars[count]);
      count += 1;
    }
  }
  ...
  // Classic RPRM
  if (data_.optimization_target().list_of_target_rpbmr_reactions().size() != 0) {
    std::cout << "CIAO" << std::endl;
    // lnA
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_rpbmr_reactions().size(); i++) {
      std::cout << "CIAO" << std::endl;
      ChangeRPBRMReactions("lnA", data_.optimization_target().list_of_target_rpbmr_reactions()[i], c_vars[count], i);
      count += 1;
    }
    // E_over_R
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_rpbmr_reactions().size(); i++) {
      ChangeRPBRMReactions("E_over_R", data_.optimization_target().list_of_target_rpbmr_reactions()[i], c_vars[count],
                           i);
      count += 1;
    }
    // Beta
    for (unsigned int i = 0; i < data_.optimization_target().list_of_target_rpbmr_reactions().size(); i++) {
      ChangeRPBRMReactions("Beta", data_.optimization_target().list_of_target_rpbmr_reactions()[i], c_vars[count], i);
      count += 1;
    }
  }
}
...
void SimulationsInterface::ChangePLOGReactions(std::string type, unsigned int index, double parameter) {
  ...
  int pos_classic_plog_reaction = std::find(indices_of_classic_plog.begin(), indices_of_classic_plog.end(), index) -
                                  indices_of_classic_plog.begin();
  ...
  if (type == "E_over_R") {
    for (int k = 0; k < data_.kineticsMapXML_->pressurelog_reactions(pos_classic_plog_reaction).E_over_R().size();
         k++) {
      double E_over_R_nominal =
          data_.nominalkineticsMapXML_->pressurelog_reactions(pos_classic_plog_reaction).E_over_R()[k][0];
...
void SimulationsInterface::ChangeRPBRMReactions(std::string type, unsigned int index, double parameter,
                                                unsigned int index_coll) {
  // std::cout << "CIAO idx " << index << std::endl;
  // int pos_rpbrm_reaction =
  //     std::find(data_.nominalkineticsMapXML()->IndicesOfRPBMR().begin(),
  ...
  //     data_.nominalkineticsMapXML_->rpbrm_reactions(pos_rpbrm_reaction).i_colliders();
  // int iSpecies =
  //     data_.thermodynamicsMapXML_->IndexOfSpecies(
  //         data_.optimization_target().list_of_target_rpbmr_bathgases()[index_coll]) -
  //     1;
  // int pos_rpbrm_coll =
  //     std::find(coll_idx.begin(), coll_idx.end(), iSpecies) - coll_idx.begin();
  // std::cout << pos_rpbrm_coll << std::endl;
  //
  // std::cout << "size: "
  //           << data_.kineticsMapXML_->rpbrm_reactions(pos_rpbrm_reaction)
  //                  .kappaMu_PLOG(pos_rpbrm_coll)
  //                  .lnA()[0][0]
  //           << std::endl;
  // if (type == "lnA") {
  ...
  //        k++) {
  //     double lnA_nominal =
  //         data_.nominalkineticsMapXML_->rpbrm_reactions(pos_rpbrm_reaction)
  //             .kappaMu_PLOG(pos_rpbrm_coll)
  //             .lnA()[k][0];
  //     double new_lnA_ = lnA_nominal + parameter * std::log(10);
  //     data_.kineticsMapXML_->rpbrm_reactions(pos_rpbrm_reaction)
  //         .kappaMu_PLOG(pos_rpbrm_coll)
  //         .Set_lnA(k, 0, new_lnA_);
  //
  //     std::cout << "Old A: " << std::exp(lnA_nominal) << std::endl;
  ...
  //     data_.kineticsMapXML_->rpbrm_reactions(pos_rpbrm_reaction)
  //         .kappaMu_PLOG(pos_rpbrm_coll)
  //         .Set_E_over_R(k, 0, new_E_over_R_);
  //   }
  // }
}

void SimulationsInterface::PrepareASCIIFile(std::ofstream &fOutput, const fs::path output_file_ascii,
...
void SimulationsInterface::PrintASCIIFile(std::ofstream &fOutput, const int eval_nr, const std::vector<double> &b,
                                          const double fn_val) {
