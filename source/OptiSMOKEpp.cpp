...
int main(int argc, char *argv[])
{
    ...
    else if (input.optimization_library() == "nlopt")
    {
        ...
        sim_iface_->Setup();
        opti_kinetics_->SetChemkinName(input.output_folder() / input.optimized_kinetics_folder() /
...
double NLOptFunction(const std::vector<double> &x, std::vector<double> &grad, void *my_func_data)
{
    ...
    for (unsigned int i = 0; i < x.size(); i++)
        std::cout << "\t" << std::scientific << std::setw(35) << std::left << input.param_str()[i] << std::scientific
                  << std::setw(35) << std::left << std::setprecision(5) << x[i] << std::endl;
