    ...
    try
    {
        for (unsigned int i = 0; i < input_paths_.size(); i++)
        {
            ...
            BOOST_FOREACH (boost::property_tree::ptree::value_type &node, ptree.get_child("data"))
            {
                ...
                boost::optional<std::string> uncertainty_node =
                    node.second.get_optional<std::string>("uncertainty_kind");
                if (uncertainty_node)
                {
...
void DataManager::OrderData()
{
    ...
    for (unsigned int i = 0; i < num; i++)
    {
        ...
        else
        {
            ...
            error_str.append("Batchreactor | PlugFlowreactor | PerfectlyStirredReactor | PremixedLaminarFlame1D | "
                             "CounterFlowFlame1D");
