...
void PerfectlyStirredReactor::Setup(const std::string input_file_name,
  ...
  // Read PSR reactor type
  {
    ...
    if (dictionaries(main_dictionary_name_).CheckOption("@Type") == true) {
      ...
      if (value == "Isothermal-ConstantPressure") type_ = OpenSMOKE::PERFECTLYSTIRRED_REACTOR_ISOTHERMAL_CONSTANTP;
        ...
        type_ = OpenSMOKE::PERFECTLYSTIRRED_REACTOR_NONISOTHERMAL_CONSTANTP;
  ...
  // Read inlet conditions
  {
    ...
    GetGasStatusFromDictionary(dictionaries(name_of_inlet_gas_status_subdictionary), *thermodynamicsMapXML_, T_Inlet,
                               P_Pa_Inlet, omega_Inlet);
  }
  ...
  // Read initial conditions
  {
    ...
    if (dictionaries(main_dictionary_name_).CheckOption("@InitialStatus") == true) {
      ...
      GetGasStatusFromDictionary(dictionaries(name_of_initial_gas_status_subdictionary), *thermodynamicsMapXML_,
                                 T_Initial, P_Pa_Initial, omega_Initial);
    } else {
      T_Initial     = T_Inlet;
  ...
  {
    on_the_fly_post_processing_ =
        new OpenSMOKE::OnTheFlyPostProcessing(*thermodynamicsMapXML_, *kineticsMapXML_, psr_options_->output_path());
  ...
  if (type_ == OpenSMOKE::PERFECTLYSTIRRED_REACTOR_NONISOTHERMAL_CONSTANTP) {
    psr_non_isothermal_ = new OpenSMOKE::PerfectlyStirredReactor_NonIsothermal_ConstantPressure(
        *thermodynamicsMapXML_, *kineticsMapXML_, *ode_parameters_, *psr_options_, *onTheFlyROPA_,
        *on_the_fly_post_processing_, *polimi_soot_, *oscillating_psr_, T_Initial, P_Pa_Initial, omega_Initial, T_Inlet,
        P_Pa_Inlet, omega_Inlet, residence_time, volume, mass_flow_rate, global_thermal_exchange_coefficient,
        exchange_area, T_environment);
  }

  else if (type_ == OpenSMOKE::PERFECTLYSTIRRED_REACTOR_ISOTHERMAL_CONSTANTP) {
    psr_isothermal_ = new OpenSMOKE::PerfectlyStirredReactor_Isothermal_ConstantPressure(
        *thermodynamicsMapXML_, *kineticsMapXML_, *ode_parameters_, *psr_options_, *onTheFlyROPA_,
        *on_the_fly_post_processing_, *polimi_soot_, *oscillating_psr_, T_Initial, P_Pa_Initial, omega_Initial, T_Inlet,
        P_Pa_Inlet, omega_Inlet, residence_time, volume, mass_flow_rate);
  }
}

void PerfectlyStirredReactor::Solve() {
  if (type_ == OpenSMOKE::PERFECTLYSTIRRED_REACTOR_ISOTHERMAL_CONSTANTP) psr_isothermal_->Solve(tEnd);
  else if (type_ == OpenSMOKE::PERFECTLYSTIRRED_REACTOR_NONISOTHERMAL_CONSTANTP) psr_non_isothermal_->Solve(tEnd);
}
...
std::vector<double> PerfectlyStirredReactor::GetMolefractionsOut(std::vector<std::string> targets_names) {
  ...
  for (unsigned int i = 0; i < targets_names.size(); i++) {
    if (targets_names[i] == "DeltaTemp" || targets_names[i] == "Temp") Mole_frac_temp[i] = T_Final;
    else Mole_frac_temp[i] = x_Final(thermodynamicsMapXML_->IndexOfSpecies(targets_names[i]));
  }
