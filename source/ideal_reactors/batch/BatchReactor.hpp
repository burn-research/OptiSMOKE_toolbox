...
void BatchReactor::Setup(const std::string input_file_name, OpenSMOKE::ThermodynamicsMap_CHEMKIN *thermodynamicsMapXML,
                         OpenSMOKE::KineticsMap_CHEMKIN *kineticsMapXML)
{
    ...
    // Read initial conditions
    {
        ...
        GetGasStatusFromDictionary(dictionaries(name_of_gas_status_subdictionary), *thermodynamicsMapXML, T, P_Pa,
                                   omega);
    }
    ...
    // Read volume
    {
        if (dictionaries(main_dictionary_name_).CheckOption("@Volume") == true)
        {
            ...
            if (units == "m3")
                ...
                OpenSMOKE::FatalErrorMessage("Unknown volume units");
        }
    }
    ...
    double exchange_area = 0.;
    {
        ...
        if (dictionaries(main_dictionary_name_).CheckOption("@ExchangeArea") == true)
        {
            ...
            if (units == "m2")
                ...
                exchange_area = value / 1.e2;
            else if (units == "cm2")
                exchange_area = value / 1.e4;
            else if (units == "mm2")
                exchange_area = value / 1.e6;
            else
    ...
    double global_thermal_exchange_coefficient = 0.;
    {
        ...
        if (dictionaries(main_dictionary_name_).CheckOption("@GlobalThermalExchangeCoefficient") == true)
        {
            ...
            if (units == "W/m2/K")
                global_thermal_exchange_coefficient = value;
    ...
    {
        onTheFlyCEMA =
            new OpenSMOKE::OnTheFlyCEMA(*thermodynamicsMapXML, *kineticsMapXML, batch_options->output_path());
    ...
    // On the fly PostProcessing
    {
        on_the_fly_post_processing =
            new OpenSMOKE::OnTheFlyPostProcessing(*thermodynamicsMapXML, *kineticsMapXML, batch_options->output_path());
...

    {
        ...
        if (dictionaries(main_dictionary_name_).CheckOption("@PressureCoefficient") == true)
        {
            ...
            batchreactor_volumeprofile->SetType(OpenSMOKE::BatchReactor_VolumeProfile::BatchReactor_VolumeProfile_Type::
            ...
            if (type_ != OpenSMOKE::BATCH_REACTOR_NONISOTHERMAL_USERDEFINEDVOLUME)
                OpenSMOKE::FatalErrorMessage(
                    "The @VolumeProfile can be used only for NonIsothermal-UserDefinedVolume reactors");
            ...
            if (units == "Pa/s")
                batchreactor_volumeprofile->SetPressureCoefficient(value);
            else if (units == "bar/s")
                ...
                OpenSMOKE::FatalErrorMessage("Unknown pressure coefficient units. Available: Pa/s || Pa/ms || bar/s || "
                                             "bar/ms || atm/s || atm/ms");
        ...
        if (dictionaries(main_dictionary_name_).CheckOption("@PressureProfile") == true)
        {
            ...
            if (type_ != OpenSMOKE::BATCH_REACTOR_NONISOTHERMAL_USERDEFINEDVOLUME)
                OpenSMOKE::FatalErrorMessage(
                    "The @PressureProfile can be used only for NonIsothermal-UserDefinedVolume reactors");
            ...
            if (y[1] != P_Pa)
                OpenSMOKE::FatalErrorMessage(
                    "The @PressureProfile and the initial pressure of mixture must be consistent");
        ...
        if (dictionaries(main_dictionary_name_).CheckOption("@TemperatureProfile") == true)
        {
            ...
            if (type_ != OpenSMOKE::BATCH_REACTOR_NONISOTHERMAL_USERDEFINEDVOLUME)
                OpenSMOKE::FatalErrorMessage(
                    "The @TemperatureProfile can be used only for NonIsothermal-UserDefinedVolume reactors");
            ...
            if (y_variable != "temperature")
                OpenSMOKE::FatalErrorMessage("The @TemperatureProfile must be defined versus the temperature");

            if (y[1] != T)
                OpenSMOKE::FatalErrorMessage(
                    "The @TemperatureProfile and the initial temperature of mixture must be consistent");
    ...
    {
        batch_nonisothermal_constantv_ = new OpenSMOKE::BatchReactor_NonIsothermal_ConstantVolume(
            *thermodynamicsMapXML, *kineticsMapXML, *ode_parameters, *batch_options, *onTheFlyROPA, *onTheFlyCEMA,
            *on_the_fly_post_processing, *idt, *polimi_soot, volume, T, P_Pa, omega,
    ...
    if (type_ == OpenSMOKE::BATCH_REACTOR_NONISOTHERMAL_USERDEFINEDVOLUME)
    {
        batch_nonisothermal_userdefinedv_ = new OpenSMOKE::BatchReactor_NonIsothermal_UserDefinedVolume(
            *thermodynamicsMapXML, *kineticsMapXML, *ode_parameters, *batch_options, *onTheFlyROPA, *onTheFlyCEMA,
            *on_the_fly_post_processing, *idt, *polimi_soot, volume, T, P_Pa, omega, tStart_,
            global_thermal_exchange_coefficient, exchange_area, T_environment);
    ...
    {
        batch_nonisothermal_constantp_ = new OpenSMOKE::BatchReactor_NonIsothermal_ConstantPressure(
            *thermodynamicsMapXML, *kineticsMapXML, *ode_parameters, *batch_options, *onTheFlyROPA, *onTheFlyCEMA,
            *on_the_fly_post_processing, *idt, *polimi_soot, volume, T, P_Pa, omega,
            global_thermal_exchange_coefficient, exchange_area, T_environment);
    }
    ...
    {
        batch_isothermal_constantp_ = new OpenSMOKE::BatchReactor_Isothermal_ConstantPressure(
            *thermodynamicsMapXML, *kineticsMapXML, *ode_parameters, *batch_options, *onTheFlyROPA, *onTheFlyCEMA,
            *on_the_fly_post_processing, *idt, *polimi_soot, volume, T, P_Pa, omega);
    }
}
...
double BatchReactor::GetIgnitionDelayTime(std::string criterion)
{
    ...
    else if (token[1] == "max" && token.size() == 3)
    {
        ...
        else if (token[2] == "intercept")
        {
            ...
            if (std::find(species_index_temp.begin(), species_index_temp.end(),
                          thermodynamicsMapXML_->IndexOfSpecies(token[0]) - 1) == species_index_temp.end())
                OptiSMOKE::FatalErrorMessage("Species: " + token[0] +
    ...
    else if (token[1] == "min" && token.size() == 3)
    {
        ...
        int species_index =
            std::distance(species_index_temp.begin(), std::find(species_index_temp.begin(), species_index_temp.end(),
                                                                thermodynamicsMapXML_->IndexOfSpecies(token[0]) - 1));
