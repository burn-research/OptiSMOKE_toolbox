...
void InputManager::ReadMainDictionary()
{
    ...
    {
        OptiSMOKE::FatalErrorMessage("Please provide the kinetic mechanism through one of the following keywords: "
                                     "@KineticsFolder | @KineticsPreProcessor");
    }
...
void InputManager::SetUpNLOPT()
{
    ...
    std::transform(initial_values_str.begin(), initial_values_str.end(), initial_values_.begin(),
...
void InputManager::ComputeBoundaries()
{
    ...
    std::vector<double> list_of_nominal_E_over_R_double(
        optimization_target_.list_of_target_uncertainty_factors().size());
    ...
    if (optimization_setup_.parameter_boundaries() == "Furst")
    {
        for (unsigned int i = 0; i < optimization_target_.list_of_target_uncertainty_factors().size(); i++)
        {
            ...
            Beta_1[i] = (kappa_upper_T_low[i] - kappa_lower_T_high[i] -
                         list_of_nominal_E_over_R_double[i] * (1 / T_high - 1 / T_low)) /
                        (std::log(T_low) - std::log(T_high));
            Beta_2[i] = (kappa_lower_T_low[i] - kappa_upper_T_high[i] -
                         list_of_nominal_E_over_R_double[i] * (1 / T_high - 1 / T_low)) /
                        (std::log(T_low) - std::log(T_high));
            ...
            lnA_1[i] = (kappa_lower_T_high[i] - (T_low / T_high) * kappa_upper_T_low[i] -
                       ...
                       (1 - (T_low / T_high));
            E_over_R_1[i] = lnA_1[i] * T_low + T_low * list_of_nominal_Beta_double[i] * std::log(T_low) -
                            kappa_upper_T_low[i] * T_low;
            lnA_2[i] = (kappa_upper_T_high[i] - (T_low / T_high) * kappa_lower_T_low[i] -
                        list_of_nominal_Beta_double[i] * (std::log(T_high) - (T_low / T_high) * std::log(T_low))) /
                       (1 - (T_low / T_high));
            E_over_R_2[i] = lnA_2[i] * T_low + T_low * list_of_nominal_Beta_double[i] * std::log(T_low) -
                            kappa_lower_T_low[i] * T_low;
            ...
            if (std::find(optimization_target_.list_of_target_E_over_R().begin(),
                ...
                optimization_target_.list_of_target_E_over_R().end())
            {
                list_of_min_abs_E_over_R_.push_back(
                    boost::lexical_cast<std::string>(list_of_min_abs_E_over_R_double[i]));
                list_of_max_abs_E_over_R_.push_back(
                    boost::lexical_cast<std::string>(list_of_max_abs_E_over_R_double[i]));
            }
        }
        ...
        for (unsigned int i = 0; i < optimization_target_.list_of_target_uncertainty_factors_inf().size(); i++)
        {
            ...
            int pos_FallOff_Reaction =
                std::find(indices_of_falloff_reactions.begin(), indices_of_falloff_reactions.end(),
            ...
            if (std::find(optimization_target_.list_of_target_Beta_inf().begin(),
                ...
                optimization_target_.list_of_target_Beta_inf().end())
            {
                list_of_min_abs_Beta_inf_.push_back(
                    boost::lexical_cast<std::string>(list_of_min_abs_Beta_inf_double[i]));
                list_of_max_abs_Beta_inf_.push_back(
                    boost::lexical_cast<std::string>(list_of_max_abs_Beta_inf_double[i]));
            }

            // Calculting extreame values of E_over_R
            lnA_1_inf[i] =
                (kappa_lower_T_high_inf[i] - (T_low / T_high) * kappa_upper_T_low_inf[i] -
            ...
            lnA_2_inf[i] =
                (kappa_upper_T_high_inf[i] - (T_low / T_high) * kappa_lower_T_low_inf[i] -
                 list_of_nominal_Beta_inf_double[i] * (std::log(T_high) - (T_low / T_high) * std::log(T_low))) /
                (1 - (T_low / T_high));
    ...
    if (optimization_setup_.parameter_boundaries() == "Narrow")
    {
        ...
        for (unsigned int i = 0; i < optimization_target_.list_of_target_uncertainty_factors_inf().size(); i++)
        {
            ...
            // Calculating extreme values for Beta
            Beta_1_inf[i] = list_of_nominal_Beta_inf_double[i] +
                            std::log(std::pow(10, optimization_target_.list_of_target_uncertainty_factors_inf()[i])) /
                                std::log(T_high);
            Beta_2_inf[i] = list_of_nominal_Beta_inf_double[i] -
                            std::log(std::pow(10, optimization_target_.list_of_target_uncertainty_factors_inf()[i])) /
                                std::log(T_high);
            ;
            list_of_min_abs_Beta_inf_double[i] = std::min(Beta_1_inf[i], Beta_2_inf[i]);
...
void InputManager::TargetsPreliminaryOptions()
{
    ...
    for (int i = 0; i < optimization_target_.list_of_target_Beta().size(); i++)
    {
        ...
        if (optimization_target_.list_of_min_rel_Beta().size() > 0)
        {
            lower_bounds_string_ += boost::lexical_cast<std::string>(
                                        (kineticsMapXML_->Beta(optimization_target_.list_of_target_Beta()[i] - 1)) *
    ...
    for (int i = 0; i < optimization_target_.list_of_target_thirdbody_reactions().size(); i++)
    {
        name_vec_thirdbody[i] = "'M_R" + std::to_string(optimization_target_.list_of_target_thirdbody_reactions()[i]) +
                                "_" + optimization_target_.list_of_target_thirdbody_species()[i] + "'";
    ...
    for (int i = 0; i < optimization_target_.list_of_target_classic_plog_reactions().size(); i++)
    {
        name_vec_lnA_classic_plog[i] = "'lnA_classic_PLOG_" +
