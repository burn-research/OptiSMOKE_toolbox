...
void OptimizedKinetics::WriteOptimizedMechanism() {
  ...
  CheckForFatalError(
      preprocessor_kinetics_->ReadKineticsFromASCIIFile(preprocessor_species_without_transport->AtomicTable()));
  ...
  for (unsigned int k = 0; k < NR_; k++) {
    ...
    if (preprocessor_kinetics_->reactions()[k].Tag() != PhysicalConstants::REACTION_LINDEMANN_FALLOFF &&
      ...
      if (preprocessor_kinetics_->reactions()[k].IsPressureLog() == true) {
        ...
        for (unsigned int l = 0; l < preprocessor_kinetics_->reactions()[k].plog_coefficients().size() - 2; l++) {
          ...
          reaction_data
              ...
              << kineticsMapXML_->pressurelog_reactions(pos_classic_plog_reaction).GetAdjustedCoefficients()[l];
      ...
      if (preprocessor_kinetics_->reactions()[k].Tag() == PhysicalConstants::REACTION_CHEBYSHEV) {
        ...
        if (preprocessor_kinetics_->reactions()[k].chebyshev_temperature_limits()[0] != 300 ||
          ...
          reaction_data << std::showpoint << std::fixed
                        << preprocessor_kinetics_->reactions()[k].chebyshev_temperature_limits()[0] << " "
                        << preprocessor_kinetics_->reactions()[k].chebyshev_temperature_limits()[1];
          reaction_data << " /" << std::endl;
        }
        ...
        if (preprocessor_kinetics_->reactions()[k].chebyshev_pressure_limits()[0] != 0.001 ||
          ...
          reaction_data << std::showpoint << std::fixed << std::left
                        << preprocessor_kinetics_->reactions()[k].chebyshev_pressure_limits()[0] << " "
                        << preprocessor_kinetics_->reactions()[k].chebyshev_pressure_limits()[1];
          reaction_data << " /" << std::endl;
        }
      ...
      reaction_data << std::setw(55) << std::left << reaction_string << " " << std::scientific
                    << kineticsMapXML_->A_falloff_inf(k) / preprocessor_kinetics_->reactions()[k].A_inf_conversion();
      reaction_data.precision(3);
      ...
      reaction_data << std::setw(13) << std::right
                    << kineticsMapXML_->E_over_R_falloff_inf(k) * PhysicalConstants::R_cal_mol << std::endl;
      reaction_data.unsetf(std::ios_base::floatfield);
      ...
      reaction_data.precision(6);
      reaction_data << std::right << std::scientific
                    << kineticsMapXML_->A(k) / preprocessor_kinetics_->reactions()[k].A_conversion();
      reaction_data.precision(6);
      reaction_data.width(14);
      reaction_data << std::fixed << std::right << kineticsMapXML_->Beta(k);
      reaction_data.precision(4);
      reaction_data << std::setw(16) << std::right << kineticsMapXML_->E_over_R(k) * PhysicalConstants::R_cal_mol
                    << " /" << std::endl;
