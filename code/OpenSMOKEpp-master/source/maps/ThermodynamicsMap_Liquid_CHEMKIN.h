...
#ifndef OpenSMOKE_ThermodynamicsMap_Liquid_CHEMKIN_H
#define OpenSMOKE_ThermodynamicsMap_Liquid_CHEMKIN_H
...
namespace OpenSMOKE
{
	//!  A class to efficiently evaluate the thermodynamic properties of gas/liquid mixtures
	/*!
	...
	*/
	...
	class ThermodynamicsMap_Liquid_CHEMKIN : public ThermodynamicsMap
	{
	...
	public:
		...
		*/
		ThermodynamicsMap_Liquid_CHEMKIN(rapidxml::xml_document<>& doc);
		...
		*/
		...
		/**
		*@brief Returns the molecular weight of the liquid mixture from the liquid mole fractions
		*@param x mole fractions of species
		*@param MW the molecular weight of liquid mixture in kg/kmol
		*/
		virtual inline double LiquidMolecularWeight_From_LiquidMoleFractions(const double* x);

		/**
		*@brief Returns the molecular weight of the liquid mixture from the liquid mass fractions
		*@param y mass fractions of liquid species
		*@param MW the molecular weight of liquid mixture in kg/kmol
		*/
		virtual inline double LiquidMolecularWeight_From_LiquidMassFractions(const double* y);

		/**
		*@brief Calculates the liquid mass fractions and the liquid molecular weight from the liquid mole fractions
		*/
		virtual inline void LiquidMassFractions_From_LiquidMoleFractions(double* y, double& MW, const double* x);
		
		/**
		*@brief Calculates the liquid mole fractions and the molecular weight from the liquid mass fractions
		*/	
		virtual inline void LiquidMoleFractions_From_LiquidMassFractions(double* x, double& MW, const double* y);
		...
		*/
        ...
        /**
		*@brief Calculates the standard specific heats of species at constant pressure (only for the liquid species)
		*@return the specific heats at constant pressure in J/kmol/K
		*/
		virtual void cpMolar_LiquidSpecies(double* cp_liquidspecies);

		/**
		*@brief Calculates the standard enthalpies of species (only for the liquid species)
		*@return the standard enthalpies in J/kmol
		*/
		virtual void hMolar_LiquidSpecies(double* h_liquidspecies);

		/**
		*@brief Calculates the standard entropies of species (only for the liquid species)
		*@return the standard entropies in J/kmol/K
		*/
		virtual void sMolar_LiquidSpecies(double* s_liquidspecies);
		...
		*/
		...
		/**
		*@brief Calculates the standard internal energies of species (only for liquid species)
		*@return the standard entropies in J/kmol
		*/
		virtual void uMolar_LiquidSpecies(double* u_species);

		/**
		*@brief Calculates the standard Gibb's free energies of species (only for liquid species)
		*@return the standard Gibb's free energies in J/kmol
		*/
		virtual void gMolar_LiquidSpecies(double* g_species);

		/**
		*@brief Calculates the standard Helmholtz's free energies of species (only for liquid species)
		*@return the standard Helmholtz's free energies in J/kmol
		*/
		virtual void aMolar_LiquidSpecies(double* a_species);
		...
		*/
		...
		/**
		*@brief Returns the total number of liquid species (i.e. all the materials are accounted for)
		*/
		inline unsigned int number_of_liquid_species() const { return number_of_liquid_species_; }

		/**
		*@brief Returns the names of all the liquid species (i.e. all the materials are accounted for)
		        The returned vector is 0-index based
		*/
		inline const std::vector<std::string>& vector_names_liquid_species() const { return vector_names_liquid_species_; }

		/**
		*@brief Returns the names of the liquid species for each material
		        Example: matrix_names_liquid_species()[k][j] returns the j-th species of the k-th material
				Be careful: both the indices runs from 0, i.e. the first material is the material number 0
 		*/
		inline const std::vector< std::vector<std::string> >&	matrix_names_liquid_species()	const {return matrix_names_liquid_species_;}
		
		/**
		*@brief Returns the liquid indices (running from 1 to the total number of liquid species) of the liquid species for each material
		        Example: matrix_indices_liquid_species()[k][j] returns the j-th species of the k-th material
 		...
 		*/
		inline const std::vector< std::vector<unsigned int>  >&	matrix_indices_liquid_species()		const {return matrix_indices_liquid_species_;}
	...
	private:
		...
		unsigned int number_of_liquid_species_;
		unsigned int number_of_gas_species_;

		std::vector<std::string>					vector_names_liquid_species_;
		std::vector< std::vector<std::string> >		matrix_names_liquid_species_;
		std::vector< std::vector<unsigned int> >	matrix_indices_liquid_species_;

	};
}

#include "ThermodynamicsMap_Liquid_CHEMKIN.hpp"

#endif /* OpenSMOKE_ThermodynamicsMap_Liquid_CHEMKIN_H */
