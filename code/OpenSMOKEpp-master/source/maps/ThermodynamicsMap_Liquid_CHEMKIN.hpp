...
namespace OpenSMOKE
{ 
	ThermodynamicsMap_Liquid_CHEMKIN::ThermodynamicsMap_Liquid_CHEMKIN(rapidxml::xml_document<>& doc)
	{
	...
	void ThermodynamicsMap_Liquid_CHEMKIN::ImportSpeciesFromXMLFile(rapidxml::xml_document<>& doc)
	{
		// Names of species
		{
			...
			rapidxml::xml_node<>* number_of_liquid_species_node = opensmoke_node->first_node("NumberOfLiquidSpecies");

			rapidxml::xml_node<>* names_of_liquid_species_node = opensmoke_node->first_node("LiquidSpecies");
			...
			try
			{
				...
				number_of_liquid_species_ = boost::lexical_cast<unsigned int>(boost::trim_copy(std::string(number_of_liquid_species_node->value())));
				...
				// Liquid species
				{
					...
					std::stringstream names_liquid_species_string;
					names_liquid_species_string << names_of_liquid_species_node->value();
					for(unsigned int i=0;i<number_of_liquid_species_;i++)
						names_liquid_species_string >> vector_names_liquid_species_[i]; 	
				}
			}
			catch(...)
			{
		...
		// Reading all the available materials
		{
			...
			for (rapidxml::xml_node<> *current_node = opensmoke_node->first_node("MaterialDescription"); current_node; current_node = current_node->next_sibling("MaterialDescription"))
			{
				...
				{
					dummy >> matrix_names_liquid_species_[k][i];
					dummy >> matrix_indices_liquid_species_[k][i];
				}
	...
	void ThermodynamicsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile(rapidxml::xml_document<>& doc)
	{
		...
		if (thermodynamics_node != 0)
		{
			...
			if (thermodynamics_type == "NASA")
			{
				...
				ErrorMessage("ThermodynamicsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile", "Thermodynamics type not supported!"); 
		}
		else
			ErrorMessage("ThermodynamicsMap_Liquid_CHEMKIN::ImportCoefficientsFromXMLFile", "Thermodynamics tag was not found!");
	}
