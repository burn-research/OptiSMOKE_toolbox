...
namespace OpenSMOKE
{
		...
		for (rapidxml::xml_node<> *kinetics_node = parent_kinetics_node->first_node("MaterialKinetics"); kinetics_node; kinetics_node = kinetics_node->next_sibling("MaterialKinetics"))
		{
			if (target_material == boost::lexical_cast<unsigned int>(kinetics_node->first_attribute("index")->value()) )
			{
				...
				// Stick reactions
				{
					...
					std::stringstream fInput;
					fInput << current_node->value();
					Load(indices_of_stick_reactions__, fInput, OPENSMOKE_FORMATTED_FILE);
				...
				// Cov dependent reactions
				{
					...
					std::stringstream fInput;
					fInput << current_node->value();
					Load(indices_of_coverage_dependent_reactions__, fInput, OPENSMOKE_FORMATTED_FILE);
				...
				// Langmuir-Hinshelwood reactions
				{
					...
					std::stringstream fInput;
					fInput << current_node->value();
					Load(indices_of_langmuir_reactions__, fInput, OPENSMOKE_FORMATTED_FILE);
				...
				// Lumped reactions
				{
					...
					std::stringstream fInput;
					fInput << current_node->value();
					Load(indices_of_lumped_reactions__, fInput, OPENSMOKE_FORMATTED_FILE);
				...
				if (type_of_kinetics_ == TYPE_OF_KINETICS_UBI_QEP)
				{
					...
					ubiqep_submechanism_->ReadFromXMLFile(kinetics_node, thermodynamics_.MWs());
				}
					...
					if (number_of_stick_reactions_ != 0)
					{
						...
						for(unsigned int i=0;i<number_of_stick_reactions_;i++)
						{
							...
							stick_constant_coefficient_[i] = sigma_power * 
															sqrt(PhysicalConstants::R_J_kmol/2./PhysicalConstants::pi/thermodynamics_.MW(stick_gas_species-1));
						}
					}
	...
	void UBIQEP_SubMechanism::ReadFromXMLFile(rapidxml::xml_node<> *xml_node, const std::vector<double>& MWs)
	{
