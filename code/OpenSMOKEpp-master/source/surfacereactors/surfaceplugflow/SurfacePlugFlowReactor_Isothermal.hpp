...
namespace OpenSMOKE
{
		...
		DAESystem_BzzDae_SurfacePlugFlowReactor* ptDae_SurfacePlugFlowReactor_Isothermal;
		void DAE_Print_SurfacePlugFlowReactor_Isothermal(BzzVector &Y, double t)
		{
			ptDae_SurfacePlugFlowReactor_Isothermal->MyPrint(Y,t);
		}
	
	#endif

	SurfacePlugFlowReactor_Isothermal::SurfacePlugFlowReactor_Isothermal(	
	...
	{
		type_ = SURFACEPLUGFLOW_REACTOR_ISOTHERMAL;
		...
		NE_DAE_ = NC_ + SURF_NC_ + SURF_NP_ + 2 + BULK_NC_;
	...
	void SurfacePlugFlowReactor_Isothermal::SetTemperatureProfile(OpenSMOKE::SurfacePlugFlowReactor_Profile&  profile)
	{
		fixed_temperature_profile_ = true;
		temperature_profile_ = &profile;
	}

	int SurfacePlugFlowReactor_Isothermal::OdeEquations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		// Recover unknowns
		unsigned int k=1;
	...
	int SurfacePlugFlowReactor_Isothermal::DaeEquations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		unsigned int k=1;
		#ifdef CHECK_MASSFRACTIONS
		...
		// Recovering residuals
		{
			...
			for (unsigned int i = 1; i <= NC_; ++i)
				dy[k++] = (thermodynamicsMap_.MW(i-1) * RfromGas_[i] +
					-omega_[i] * dG_over_dz +
					RfromSurface_[i] / cross_section_over_perimeter_ * thermodynamicsMap_.MW(i-1)) / rho_;
			...
			// Phases
			for (unsigned int i = 1; i <= SURF_NP_; ++i)
				dy[k++] = RsurfacePhases_[i]*artificial_speedup_coefficient_;
			...
			// Independent coordinate
			if (time_independent_ == false)	dy[k++] = 1.;
			else                            dy[k++] = v_;
	...
	int SurfacePlugFlowReactor_Isothermal::OdePrint(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		ode_iteration_++;
	...
	int SurfacePlugFlowReactor_Isothermal::DaePrint(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		dae_iteration_++;
			...
			if (dae_iteration_%plugflow_options_.n_step_video() == 1 || plugflow_options_.n_step_video() == 1)
			{
				...
				if (counter_file_video_ % 100 == 1)
				{
					...
					if (time_independent_ == true)
						...
						std::cout << std::setw(18) << std::left << "Space[m]";
				...
				std::cout << std::fixed << std::setw(9) << std::left << dae_iteration_;
				std::cout << std::scientific << std::setw(18) << std::setprecision(6) << std::left << t;
				...
				if (BULK_NC_ != 0)
					std::cout << std::scientific << std::setw(16) << std::setprecision(6) << std::left << thicknessBulk_.SumElements()*1.e6;
				std::cout << std::endl;
			}
		}
	...
	void SurfacePlugFlowReactor_Isothermal::PrintCurrentStatus(std::ostream& fOutput, std::ostream& fBulkOutput)
	{
	...
	void SurfacePlugFlowReactor_Isothermal::PrintParametricFinalStatus(std::ostream& fOutput)
	{
		fOutput.setf(std::ios::scientific);
		fOutput << std::setw(20) << std::left << tau_;
		fOutput << std::setw(20) << std::left << csi_;
	...
	void SurfacePlugFlowReactor_Isothermal::SetInletConditions()
	{
		Gamma0_ = Gamma_;
		Z0_ = Z_;
		thicknessBulk0_ = 0.;
	...
	void SurfacePlugFlowReactor_Isothermal::Solve(const double tf)
	{
		if (plugflow_options_.verbose_video() == true)
		{
			std::cout << std::endl;
			std::cout << "-----------------------------------------------------------------------------" << std::endl;
			std::cout << " Solving the isothermal plug flow reactor...                                 " << std::endl;
			std::cout << "-----------------------------------------------------------------------------" << std::endl;
		}
		...
		else if (dae_parameters_.type() == DAE_Parameters::DAE_INTEGRATOR_BZZDAE)
		{
			...
			// Initial conditions
			BzzVector y0_bzz(yDae0_.Size());
			yDae0_.CopyTo(y0_bzz.GetHandle());
			...
			BzzDaeObject o(y0_bzz, 0., iDerAlg, &daeplugflow);
			...
			ptDae_SurfacePlugFlowReactor_Isothermal = &daeplugflow;
			o.StepPrint(DAE_Print_SurfacePlugFlowReactor_Isothermal);

			double tStart = OpenSMOKE::OpenSMOKEGetCpuTime();
			yf_bzz = o(tf,tf);
			double tEnd = OpenSMOKE::OpenSMOKEGetCpuTime();
			yDaef_.CopyFrom(yf_bzz.GetHandle());
	...
	void SurfacePlugFlowReactor_Isothermal::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		FatalErrorMessage("SurfacePlugFlowReactor_Isothermal::SensitivityAnalysis not yet implemented");
