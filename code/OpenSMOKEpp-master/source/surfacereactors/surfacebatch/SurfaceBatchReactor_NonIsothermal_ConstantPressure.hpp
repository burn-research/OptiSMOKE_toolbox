...
namespace OpenSMOKE
{
	...
	ODESystem_BzzOde_SurfaceBatchReactor* ptBzzOde_SurfaceBatchReactor_NonIsothermal_ConstantPressure;
	void ODE_Print_SurfaceBatchReactor_NonIsothermal_ConstantPressure(BzzVector &Y, double t)
	{
		ptBzzOde_SurfaceBatchReactor_NonIsothermal_ConstantPressure->MyPrint(Y,t);
	}
	#endif


	SurfaceBatchReactor_NonIsothermal_ConstantPressure::SurfaceBatchReactor_NonIsothermal_ConstantPressure(	
	...
	{
		type_ = SURFACEBATCH_REACTOR_NONISOTHERMAL_CONSTANTP;
	...
	int SurfaceBatchReactor_NonIsothermal_ConstantPressure::Equations(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y, OpenSMOKE::OpenSMOKEVectorDouble& dy)
	{
		// Recover unknowns
		{
			unsigned int k=1;
		...
		// Recovering residuals
		{
			...
			// Energy equation
			dy[k++] = (V_*QRGas_+ A_*QRSurface_) / (mass_*CpMixMass_);
		}

		return 0;
	}

	int SurfaceBatchReactor_NonIsothermal_ConstantPressure::Print(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		iteration_++;
	...
	void SurfaceBatchReactor_NonIsothermal_ConstantPressure::Solve(const double tf)
	{
		if (batch_options_.verbose_output() == true)
		{
			std::cout << std::endl;
		...
		else if (ode_parameters_.type() == ODE_Parameters::ODE_INTEGRATOR_BZZODE)
		{
			...
			ptBzzOde_SurfaceBatchReactor_NonIsothermal_ConstantPressure = &odebatch;
			o.StepPrint(ODE_Print_SurfaceBatchReactor_NonIsothermal_ConstantPressure);
	...
	void SurfaceBatchReactor_NonIsothermal_ConstantPressure::SensitivityAnalysis(const double t, const OpenSMOKE::OpenSMOKEVectorDouble& y)
	{
		FatalErrorMessage("SurfaceBatchReactor_NonIsothermal_ConstantPressure::SensitivityAnalysis not yet implemented");
