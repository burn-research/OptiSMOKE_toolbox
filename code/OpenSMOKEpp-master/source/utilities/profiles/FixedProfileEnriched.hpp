...
namespace OpenSMOKE
{
	FixedProfileEnriched::FixedProfileEnriched(const std::string file_name)
	{
		...
		if (line_filter != -1)
		{
			...
			// Filtering
			{
				OpenSMOKE::OpenSMOKEVectorDouble xo(x.size());
				OpenSMOKE::OpenSMOKEVectorDouble yo(x.size());
				...
				OpenSMOKE::OpenSMOKEVectorDouble x_new(x.size());
				OpenSMOKE::OpenSMOKEVectorDouble y_new(x.size());
		...
		fixed_profile_ = new FixedProfile(x.size(), x.data(), y.data());
	}
}
