			...
			// Assembling
			{
				...
				for (unsigned int j = 1; j <= number_of_parameters_; j++)
				{
					...
					if (index_of_temperature_ == 0 && index_of_density_ == 0)
						kinetics_.SensitivityWithRespectKineticParameter(sensitivity_type_, j, c.GetHandle(), Jp_species_.GetHandle(), parameters_[j]);
					else if (index_of_temperature_ != 0 && index_of_density_ == 0)
						kinetics_.SensitivityWithRespectKineticParameter(sensitivity_type_, energy_type_, j, c.GetHandle(), mole_fractions.GetHandle(), Jp_species_.GetHandle(), Jp_T_, parameters_[j]);
					else if (index_of_temperature_ != 0 && index_of_density_ != 0)
						kinetics_.SensitivityWithRespectKineticParameter(sensitivity_type_, energy_type_, j, c.GetHandle(), mole_fractions.GetHandle(), Jp_species_.GetHandle(), Jp_T_, Jp_rho_, parameters_[j]);
