...
namespace OpenSMOKE
{
	...
	{
		// Basic variables
		NE_ = thermodynamicsMap_.elements().size();
	...
	{
		cem_index_  = lambda_indices_sorted[0];
	...
	void OnTheFlyCEMA::CalculateEigen(const double t, const double T, const double P_Pa, const OpenSMOKE::OpenSMOKEVectorDouble& c, const OpenSMOKE::OpenSMOKEMatrixDouble& J)
	{
...

		{
			...
			{
				cem_index_ = right_lambda_original_associations_sorted[0];
...

		{
			...
			// Left eigenvector b
			{
				const unsigned int left_cem_index = left_lambda_original_associations_sorted[0];
