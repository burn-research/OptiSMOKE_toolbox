...
namespace OpenSMOKE
{
	...
	void PressureLogarithmicRateExpression::Setup(std::vector<double> coefficients_)
	{
		...
		// initializes first dimensions of objects containing reactions parameters 
		lnA_.resize(N);
		Beta_.resize(N);
		E_over_R_.resize(N);
		p_.resize(N);
		lnp_.resize(N);
		// taking conversion factors away from inizial string
		conversion_A_ = coefficients_[coefficients_.size()-2];
		conversion_E_ = coefficients_[coefficients_.size()-1];
