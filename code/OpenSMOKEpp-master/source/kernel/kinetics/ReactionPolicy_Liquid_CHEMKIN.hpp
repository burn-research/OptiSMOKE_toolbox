...
namespace OpenSMOKE
{
	ReactionPolicy_Liquid_CHEMKIN::ReactionPolicy_Liquid_CHEMKIN() 
	{
		SetDefaultUnits();
	}

	void ReactionPolicy_Liquid_CHEMKIN::SetDefaultUnits()
	{
		iReversible_ = false;
	...
	ReactionPolicy_Liquid_CHEMKIN::ReactionPolicy_Liquid_CHEMKIN(const ReactionPolicy_Liquid_CHEMKIN& orig) 
	{
	}
	...
	PhysicalConstants::TAG_REACTION_SURFACE ReactionPolicy_Liquid_CHEMKIN::Tag() const
	{
		return tag_reaction_;
	}

	std::string ReactionPolicy_Liquid_CHEMKIN::TagASCII() const
	{
	...
	bool ReactionPolicy_Liquid_CHEMKIN::SetUnits(const PhysicalConstants::UNITS_REACTION a_units, const PhysicalConstants::UNITS_REACTION e_units, const PhysicalConstants::UNITS_REACTION_COMPOSITION composition_units)
	{
		a_units_ = a_units;
		e_units_ = e_units;
	...
	bool ReactionPolicy_Liquid_CHEMKIN::ReadReactionFromStrings(	const std::vector<std::string>& lines, const std::map<std::string, unsigned int>& map_of_species,
																	const unsigned int number_of_gas_species, const unsigned int number_of_liquid_species)
	{
		number_of_gas_species_ = number_of_gas_species;
		number_of_liquid_species_ = number_of_liquid_species;
	...
	void ReactionPolicy_Liquid_CHEMKIN::ReactionOrders() 
	{

		sumLambdaGasReactants_  = 0.;
		sumLambdaLiquidReactants_ = 0.;
		for(unsigned int i=0;i<reactant_lambda_.size();i++)
		{
			if (reactant_lambda_indices_[i] < number_of_gas_species_)								sumLambdaGasReactants_ += reactant_lambda_[i];
			else if (reactant_lambda_indices_[i] < number_of_gas_species_+number_of_liquid_species_)	sumLambdaLiquidReactants_ += reactant_lambda_[i];
		}

		sumLambdaGasProducts_  = 0.;
		sumLambdaLiquidProducts_ = 0.;
		for(unsigned int i=0;i<product_lambda_.size();i++)
		{
			if (product_lambda_indices_[i] < number_of_gas_species_)								sumLambdaGasProducts_  += product_lambda_[i];
			else if (product_lambda_indices_[i] < number_of_gas_species_+number_of_liquid_species_)	sumLambdaLiquidProducts_ += product_lambda_[i];
		}
	}

	void ReactionPolicy_Liquid_CHEMKIN::ConvertUnits() 
	{
		ConversionFactors();
		ReactionOrders();
		...
		{			
			if (composition_units_ == PhysicalConstants::UNITS_STD)
			{
				...
				A_ *= std::pow(1.e3, 1.-sumLambdaGasReactants_-sumLambdaLiquidReactants_)/
					std::pow(a_conversion_, 1. - sumLambdaGasReactants_ - sumLambdaLiquidReactants_);		// [m, kmol, s]
			}
			else 
			{
				...
				A_ *=	pow(conversion_factor, -sumLambdaGasReactants_)*std::pow(10., 1.-sumLambdaLiquidReactants_) /
						pow(a_conversion_, 1.-sumLambdaLiquidReactants_);									// [m, kmol, s];
			}

			if (iExplicitlyReversible_ == true)
				ARev_ *=	std::pow(1.e3, 1. - sumLambdaGasProducts_ - sumLambdaLiquidProducts_) /
							std::pow(a_conversion_, 1. - sumLambdaGasProducts_ - sumLambdaLiquidProducts_);	// [m, kmol, s]
		}
	}

	void ReactionPolicy_Liquid_CHEMKIN::ConversionFactors() 
	{
	...
	void ReactionPolicy_Liquid_CHEMKIN::GetReactionString(const std::vector<std::string>& list_species, std::string& line_reaction) const
	{
		// Reactants
		for(unsigned int i=0;i<reactant_nu_.size();i++)
		{
	...
	void ReactionPolicy_Liquid_CHEMKIN::GetReactionStringCHEMKIN(	const std::vector<std::string>& list_species, std::stringstream& reaction_data) const
	{
	...
	void ReactionPolicy_Liquid_CHEMKIN::GetReactionStringCHEMKIN(const std::vector<std::string>& list_species,
	...
	double ReactionPolicy_Liquid_CHEMKIN::A_conversion() const
	{
		double conversion_factor = 0;
		// simple
		{
			conversion_factor = std::pow(1.e-3, 1. - sumLambdaGasReactants_ - sumLambdaLiquidReactants_) /
								std::pow(1.e-4, 1. - 1.5*sumLambdaGasReactants_ - sumLambdaLiquidReactants_) /
								std::pow(a_conversion_, 1. - sumLambdaGasReactants_ - sumLambdaLiquidReactants_);
		}
            
		return conversion_factor;
	}
        
	double ReactionPolicy_Liquid_CHEMKIN::Arev_conversion() const
    {
        double conversion_factor = 0;
        if(	tag_reaction_ == PhysicalConstants::REACTION_SURFACE_SIMPLE )
        {
	...
	void ReactionPolicy_Liquid_CHEMKIN::WriteAdditionalDataOnASCIIFile(std::ostream& fOut) const
	{
		if (tag_reaction_ == PhysicalConstants::REACTION_SURFACE_SIMPLE)
		{
	...
	void ReactionPolicy_Liquid_CHEMKIN::WriteShortSummary(std::ostream& fOut, const std::vector<std::string>& list_species) const
	{
	...
	double ReactionPolicy_Liquid_CHEMKIN::GetForwardConversionFactor() const
	{
		return 1. / (	std::pow(1.e-3, 1. - sumLambdaGasReactants_ - sumLambdaLiquidReactants_) /
						std::pow(1.e-4, 1. - 1.5*sumLambdaGasReactants_ - sumLambdaLiquidReactants_));
	}

	double ReactionPolicy_Liquid_CHEMKIN::GetBackwardConversionFactor() const
	{
		return 1. / (	std::pow(1.e-3, 1. - sumLambdaGasReactants_ - sumLambdaLiquidReactants_) /
						std::pow(1.e-4, 1. - 1.5*sumLambdaGasReactants_ - sumLambdaLiquidReactants_));
	}

	void ReactionPolicy_Liquid_CHEMKIN::WriteSummary(std::ofstream& fOut, const std::vector<std::string>& list_species, const unsigned int index) const
	{
		std::string line_reaction;
		GetReactionString(list_species, line_reaction);
		...
		fOut << " Reaction order (forward, liquid)  = " << std::setprecision(3) << sumLambdaLiquidReactants_ << std::endl;
		if (iReversible_ == true)
		{
			fOut << " Reaction order (backward, gas)  = " << std::setprecision(3) << sumLambdaGasProducts_ << std::endl;
			fOut << " Reaction order (backward, liquid) = " << std::setprecision(3) << sumLambdaLiquidProducts_ << std::endl;
		}
	...
	bool ReactionPolicy_Liquid_CHEMKIN::FatalErrorMessage(const std::string message)
	{
